...
async function fetchBinance(asset: string): Promise<FetchedData> {
  const SYMBOLS: SymbolMap = {
    ...
    XRP: 'XRPUSDT', DOGE: 'DOGEUSDT', ADA: 'ADAUSDT', TON: 'TONUSDT',
  };
  ...
  try {
    ...
    return {
      source: 'Binance', sourceUrl: url, asset, unit: 'USD',
      price, rawValue: price ? `$${price.toLocaleString('en-US', { maximumFractionDigits: 4 })}` : 'unavailable',
      fetchedAt: new Date().toISOString(),
    };
  } catch {
    return { source: 'Binance', sourceUrl: url, asset, unit: 'USD', price: null, rawValue: 'fetch failed', fetchedAt: new Date().toISOString() };
  }
}
...
async function fetchCoinGecko(asset: string): Promise<FetchedData> {
  ...
  try {
    ...
    return {
      source: 'CoinGecko', sourceUrl: url, asset, unit: 'USD',
      price, rawValue: price ? `$${price.toLocaleString('en-US', { maximumFractionDigits: 4 })}` : 'unavailable',
      fetchedAt: new Date().toISOString(),
    };
  } catch {
    return { source: 'CoinGecko', sourceUrl: url, asset, unit: 'USD', price: null, rawValue: 'fetch failed', fetchedAt: new Date().toISOString() };
  }
}
...
async function fetchCryptoCompare(asset: string): Promise<FetchedData> {
  ...
  try {
    ...
    return {
      source: 'Chainlink (CryptoCompare proxy)', sourceUrl: url, asset, unit: 'USD',
      price, rawValue: price ? `$${price.toLocaleString('en-US', { maximumFractionDigits: 4 })}` : 'unavailable',
      fetchedAt: new Date().toISOString(),
      note: 'CryptoCompare aggregate feed used as on-chain oracle proxy for testnet',
    };
  } catch {
    return { source: 'Chainlink (CryptoCompare proxy)', sourceUrl: url, asset, unit: 'USD', price: null, rawValue: 'fetch failed', fetchedAt: new Date().toISOString() };
  }
}
...
async function fetchCoinbase(asset: string): Promise<FetchedData> {
  ...
  try {
    const res  = await fetchWithTimeout(url);
    const data = await res.json() as { data?: { amount?: string; currency?: string } };
    const price = data.data?.amount ? parseFloat(data.data.amount) : null;
    return {
      source: 'Coinbase', sourceUrl: url, asset, unit: 'USD',
      price, rawValue: price ? `$${price.toLocaleString('en-US', { maximumFractionDigits: 4 })}` : 'unavailable',
      fetchedAt: new Date().toISOString(),
    };
  } catch {
    return { source: 'Coinbase', sourceUrl: url, asset, unit: 'USD', price: null, rawValue: 'fetch failed', fetchedAt: new Date().toISOString() };
  }
}
...
async function fetchKraken(asset: string): Promise<FetchedData> {
  const PAIRS: SymbolMap = {
    ETH: 'ETHUSD', BTC: 'XBTUSD', SOL: 'SOLUSD',
  ...
  try {
    ...
    return {
      source: 'Kraken', sourceUrl: url, asset, unit: 'USD',
      price, rawValue: price ? `$${price.toLocaleString('en-US', { maximumFractionDigits: 4 })}` : 'unavailable',
      fetchedAt: new Date().toISOString(),
    };
  } catch {
    return { source: 'Kraken', sourceUrl: url, asset, unit: 'USD', price: null, rawValue: 'fetch failed', fetchedAt: new Date().toISOString() };
  }
}
...
async function fetchOKX(asset: string): Promise<FetchedData> {
  const INST: SymbolMap = {
    ETH: 'ETH-USDT', BTC: 'BTC-USDT', SOL: 'SOL-USDT',
    XRP: 'XRP-USDT', DOGE: 'DOGE-USDT', ADA: 'ADA-USDT', TON: 'TON-USDT',
  };
  ...
  try {
    ...
    return {
      source: 'OKX', sourceUrl: url, asset, unit: 'USD',
      price, rawValue: price ? `$${price.toLocaleString('en-US', { maximumFractionDigits: 4 })}` : 'unavailable',
      fetchedAt: new Date().toISOString(),
    };
  } catch {
    return { source: 'OKX', sourceUrl: url, asset, unit: 'USD', price: null, rawValue: 'fetch failed', fetchedAt: new Date().toISOString() };
  }
}
...
async function fetchBybit(asset: string): Promise<FetchedData> {
  ...
  try {
    ...
    const price = data.result?.list?.[0]?.lastPrice
      ...
      : null;
    return {
      source: 'Bybit', sourceUrl: url, asset, unit: 'USD',
      price, rawValue: price ? `$${price.toLocaleString('en-US', { maximumFractionDigits: 4 })}` : 'unavailable',
      fetchedAt: new Date().toISOString(),
    };
  } catch {
    return { source: 'Bybit', sourceUrl: url, asset, unit: 'USD', price: null, rawValue: 'fetch failed', fetchedAt: new Date().toISOString() };
  }
}
...
async function fetchDefiLlamaBase(): Promise<FetchedData> {
  ...
  try {
    ...
    return {
      source: 'DeFiLlama', sourceUrl: url, asset: 'BASE_TVL', unit: 'USD TVL',
      price: tvl, rawValue: tvlB ? `$${tvlB}B` : 'unavailable',
      fetchedAt: new Date().toISOString(),
      note: `Base chain TVL from DeFiLlama: ${tvlB ? '$' + tvlB + 'B' : 'unavailable'}`,
    };
  } catch {
    return { source: 'DeFiLlama', sourceUrl: url, asset: 'BASE_TVL', unit: 'USD TVL', price: null, rawValue: 'fetch failed', fetchedAt: new Date().toISOString() };
  }
}
...
async function fetchMacroRates(): Promise<FetchedData> {
  ...
  const url = 'https://api.stlouisfed.org/fred/series/observations?series_id=FEDFUNDS&sort_order=desc&limit=1&file_type=json&api_key=abcdefghijklmnopqrstuvwxyz123456';
  // Fallback: use a well-known public alternative
  const fallbackUrl = 'https://api.fiscaldata.treasury.gov/services/api/v1/accounting/od/avg_interest_rates?fields=record_date,avg_interest_rate_amt&filter=security_desc:eq:Federal%20Funds&sort=-record_date&page[size]=1';
  try {
    const res  = await fetchWithTimeout(fallbackUrl);
    const data = await res.json() as { data?: Array<{ avg_interest_rate_amt?: string; record_date?: string }> };
    const latest = data.data?.[0];
    const rate   = latest?.avg_interest_rate_amt ? parseFloat(latest.avg_interest_rate_amt) : null;
    return {
      source: 'US Treasury (FiscalData)', sourceUrl: fallbackUrl,
      asset: 'MACRO_RATES', unit: 'bps',
      price: rate, rawValue: rate ? `${rate.toFixed(2)}%` : 'unavailable',
      fetchedAt: new Date().toISOString(),
    ...
    return {
      source: 'US Treasury (FiscalData)', sourceUrl: fallbackUrl,
      asset: 'MACRO_RATES', unit: 'bps', price: null,
      rawValue: 'fetch failed', fetchedAt: new Date().toISOString(),
...
function policyFallback(): FetchedData {
  return {
    source: 'EU Commission (no public API)', sourceUrl: 'https://digital-strategy.ec.europa.eu/en/policies/ai-act',
    asset: 'POLICY', unit: 'EUR fine', price: null,
