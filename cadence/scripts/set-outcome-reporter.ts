...
async function main() {
  ...
  const wallet   = new ethers.Wallet(VAULT_OWNER_KEY, provider);
  const vault    = new ethers.Contract(GHOST_VAULT_ADDRESS, ABI, wallet);
  ...
  console.log('Current reporter :', currentReporter === ethers.ZeroAddress ? '(none)' : currentReporter);
